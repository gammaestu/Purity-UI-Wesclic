        ...
        <tbody class="text-[13px]">
            @foreach ($rows as $row)
                <tr class="border-b border-slate-50 last:border-0">
                    ...
                    <td class="py-4 text-slate-600 font-semibold">{{ $row['budget'] }}</td>
                    <td class="py-4 text-slate-600 font-semibold">{{ $row['status'] }}</td>
                    <td class="py-4">
                        <div class="flex items-center gap-3">
                            <div class="flex-1 h-1.5 rounded-full bg-slate-100 overflow-hidden">
                                <div class="h-full rounded-full bg-[#4FD1C5]" style="width: {{ $row['completion'] }}%"></div>
                            </div>
                            <span class="text-xs font-semibold text-slate-600">{{ $row['completion'] }}%</span>
                        </div>
                    </td>
